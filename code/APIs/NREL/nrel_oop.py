...
class NREL_API:
    def __init__(self):
        self.key = getpass("Pass in your NREL API Key: ")

    def get_route(self, endpoint_name: str):
        self.endpoint_name = endpoint_name
        some_route = f"{self.endpoint_name}"

        return f"https://developer.nrel.gov/api/{some_route}?.json"
    ...
    def retrieve_data(self, parameters: dict):
        ...
        nrel_path = getcwd() + "/data/APIs/NatlRenewableEnergyLab"

        if route_folder not in listdir(nrel_path):
            chdir(nrel_path)
            makedirs(route_folder)
            chdir("../../..")

        # Serializing the Data with Version Control
        json_path = getcwd() + f"/data/APIs/NatlRenewableEnergyLab/{route_folder}/json"
        vc_path = getcwd() + f"/data/APIs/NatlRenewableEnergyLab/{route_folder}/version_check"
